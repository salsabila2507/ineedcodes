...
// ── CLI basics ──
{ const { code, out } = await run(['--version']); check('--version prints version', code === 0 && out.includes('ineed 1.7.13'), out); }
...
// ── session REPL ──
{
  ...
  check('session: banner shows ineed', out.includes('ineed') && out.includes('v1.7.13'), out);
  server.close();
}
